{ pkgs }: 
let
  yesitsmeSrc = pkgs.fetchFromGitHub { owner = "0x0be"; repo = "yesitsme"; rev = "64bea3936aa1a9ad29ac7eaebe93c5c4159ab1d3"; sha256 = "09g4pcmcjynqhka3y739x9ikqs8v5xi2vp9f5v19r04zbgk5vpfy"; };
  traxosintSrc = pkgs.fetchFromGitHub { owner = "N0rz3"; repo = "TraxOsint"; rev = "5a1be79ab3e7f8812d7928c571f6c69c83be7211"; sha256 = "1i6sywa5j146b7z8jkkwwmd7xg3lf8nh4smpfgdg10m6jnjclv9w"; };
  mcp-obsidianSrc = pkgs.fetchFromGitHub { owner = "MarkusPfundstein"; repo = "mcp-obsidian"; rev = "5ee0b84fa8319fd2fdf0db0ee1febb065e712a15"; sha256 = "0g4fs40zc10bgbl4s7a3l9py3nlglp7c1n60xmscg91210zha9ay"; };
  py = pkgs.python3Packages;
  scrape = py.buildPythonPackage {
    pname = "scrape-search-engine"; version = "0.2.2";
    src = pkgs.fetchurl {
    ...
    pyproject = true; build-system = [ py.setuptools ];
    dependencies = [ py.requests py.beautifulsoup4 ];
    pythonImportsCheck = [ "ScrapeSearchEngine.SearchEngine" ];
  };
  osintPython = pkgs.python3.withPackages (p: [ p.httpx p.requests p.beautifulsoup4 p.colorama p.folium scrape ]);
in {
  ...
  mcp-obsidian = py.buildPythonApplication {
    pname = "mcp-obsidian"; version = "0.2.2";
    src = mcp-obsidianSrc;
    pyproject = true; build-system = [ py.hatchling ];
    dependencies = [ py.mcp py.python-dotenv py.requests ];
    postPatch = ''
